...
- Extended `WindowManager` with focus API:
  - `activeWindow()` / `activeWindowId()` to read the current focus
  - `setActiveWindow(windowId)` to change focus
  - Exposed `activeWindowId` as a QML property for binding
- At startup, the demonstration window automatically becomes the active window.
- `WindowManager` logs focus changes:
  - `[BDE] Active window: 1`
  - `[BDE] Window 1 focused`
  - `[BDE] Window 1 lost focus`
- Updated `qml/WindowFrame.qml`:
  - Added `isActive` property and MouseArea for click detection.
  - Active window uses a slightly brighter title bar and a slightly thicker border.
  - Uses `z` ordering so the active window renders above inactive windows.
- Updated `qml/DesktopSurface.qml` to bind each frame's `isActive` to `windowManager.activeWindowId`.
- Business logic remains in C++; QML only handles input and presentation.
- No forbidden features are implemented: no dragging, resizing, minimize, maximize, close, keyboard shortcuts, snapping, window effects, animations, or context menus.
        ...
        └── bos::shell::Application
              ...
              └── SessionManager::start()
                    └── ModuleManager::startAll()
                          └── WindowManager creates demo window and sets it active
